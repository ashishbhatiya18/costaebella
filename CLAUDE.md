...
**Narrower per-route access** (first example of this pattern): every Ledgerly route uses the same `RequireAuth` as Shiftly/Pantrly *except* `GET /api/ledgerly/summary/pnl`, which is restricted to a handful of emails — data entry stays open to every admin, only the financial summary is gated. `middleware.RequireAuth` stashes the verified email in context (`AdminEmailKey`, added alongside the existing `AdminIDKey`) so `internal/ledgerly/access.RequireLedgerlyAdmin` can check it against a separate `ledgerly_admins` table (seeded from the `LEDGERLY_ADMIN_EMAILS` env var, mirroring how `ADMIN_EMAIL` seeds the main `admins` whitelist). `lib/ledgerly/use-access.ts`'s `useLedgerlyAccess()` hook probes `GET /api/ledgerly/access` once per mount; `app/ledgerly/dashboard/layout.tsx` uses it to hide the "P&L Summary" nav tab entirely for admins without access (not just block the page), and `summary/page.tsx` uses the same hook as a fallback gate for anyone who navigates there directly by URL. Either way the actual enforcement is server-side — the frontend check only avoids dangling a link or page that would 403. Any future route needing narrower visibility than the shared admin whitelist should follow this same shape rather than gating a whole app.
...
**Auth**: Google Sign-In only, no passwords. `POST /api/auth/google` exchanges a Google ID token for a JWT, but only if the resulting email exists in the `admins` table — a whitelist, not open signup, shared across every app behind `/admin`. `internal/auth/google.go`'s `VerifyGoogleIDToken` does full verification (RS256 signature against Google's JWKS, issuer, audience, `email_verified`) before anything touches the whitelist. `config.AdminEmail` (from `.env`) is upserted into that table on every boot via `SeedWhitelistedEmail` (`internal/auth/auth.go`) — to grant someone access, set `ADMIN_EMAIL` and restart the backend, or insert directly into `admins`. All other routes require `Authorization: Bearer <jwt>` via `RequireAuth`, which also stashes the verified email in context (`middleware.AdminEmailKey`) for any narrower per-route whitelist check (see Ledgerly below). Every whitelist email (`admins`, `ledgerly_admins`) is compared via `auth.NormalizeEmail` (trim + lowercase) on both insert and lookup, so incidental case/whitespace differences can't silently lock someone out or open a gap.
